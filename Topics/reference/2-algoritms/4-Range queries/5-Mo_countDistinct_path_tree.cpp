/*
given tree and q queries, each query is a path from u to v, find the number of distinct elements in the path

using eular tour and MO's algorithm.
- eular will store each node twice, once when entering and once when leaving.
- for Mo we can check if node is exiting once or twice, if it's exiting twice then we don't count it's value.
- we got to case of lca of u and v is u or v, then we can just use the range from l[u] to l[v] or r[u] to l[v] respectively.
- if lca is not u or v, then we can use the range from r[u] to l[v] and add the lca value to the range.
*/
