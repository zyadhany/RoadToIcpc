/*
Given Graph, using it's src as root we can build a dominator tree.
for each node we define dom[v] as the nearest node that dominates v.
node u dominates node v if every path from the root to v passes through u.
we can build the dominator tree as parents of each node is dom[v].
*/

/*
if Graph is acyclic, we can build the dominator tree by topological sorting the graph and iterate over the nodes and for each node v, dom[v] is lca of all the nodes that connected to v in the topological order.
*/
...
// find all points that must get visited when moving from 1 to n
void solve(int tc) {
	ll n, m;
