...
struct Query
{
    ll opp, k, l, r, val, ind;
};

vi res;
vector<vector<Query>> Y;


void DFS(SegmentTree &sg, ll n) {
    vp opp;
    for (auto neg : Y[n]) {
        if (neg.opp == 3) {
            DFS(sg, neg.l);
        }else if (neg.opp == 2) {
            res[neg.ind] = sg.getrange(neg.l, neg.r);
        }else {
            ll re = sg.getrange(neg.l, neg.l);
            opp.push_back({re, neg.l});
            sg.set(neg.l, neg.r);
        }
    }

    for (int i = opp.size() - 1; i >= 0; i--)
        sg.set(opp[i].second, opp[i].first);
}
...
void solve(ll tc) {
    ...
    {
        cin >> Q[i].opp >> Q[i].k;
        if (Q[i].opp != 3) cin >> Q[i].l >> Q[i].r;
        else Q[i].l = at++;
        Q[i].ind = i;
        Y[Q[i].k].push_back(Q[i]);
    }   
